...
function py_translate_circuit(circuit_data::Any, qubit_map::Any, connectivity_qiskit::Any)
    circuit_data = Vector{Tuple{String,Vector{Int},Vector{Any}}}([
        (
            name,
            Vector{Int}([index for index in indices]),
            Vector{Any}([parameter for parameter in parameters]),
        ) for (name, indices, parameters) in circuit_data
    ])
...
function translate_circuit(circuit_data::Any, qubit_map::Any)
    circuit_data, qubit_map, connectivity_qiskit =
        py_translate_circuit(circuit_data, qubit_map, [])
    return translate_circuit(circuit_data, qubit_map)
end
function translate_circuit(
    circuit_data::Vector{Tuple{String,Vector{Int},Vector{Any}}},
    qubit_map::Dict{Int64,Tuple{Int,Int}},
)
    list_gates = []
...
function translate_observable(obs::Any, qubit_map::Any)
    qubit_map = Dict{Int,Tuple{Int,Int}}(k => Tuple(v) for (k, v) in qubit_map)
    return Vector{Tuple{String,Vector{Tuple{Int,Int}},Float64}}([
        (term, [qubit_map[index] for index in indices], coeff) for
        (term, indices, coeff) in obs
    ])
end

function translate_observable(
    obs::Vector{Tuple{String,Vector{Int},Float64}},
    qubit_map::Dict{Int64,Tuple{Int,Int}},
)
    return Vector{Tuple{String,Vector{Tuple{Int,Int}},Float64}}([
        (term, Tuple{Int}(qubit_map[indices]), coeff) for (term, indices, coeff) in obs
    ])
end
...
function translate_samples(samples::Any, qubit_map::Any)
    ...
    for sample in samples
        tranlated_sample = join([sample[coord] for (qindex, coord) in reverse(qubit_map)])
